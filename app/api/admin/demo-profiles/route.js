...
export async function GET(request) {
  try {
    ...
    return NextResponse.json(
      { control, profiles: profiles.map(serialize) },
...
export async function POST(request) {
  ...
  try {
    ...
    if (action === "show") {
      ...
      await appendAdminAudit({
        actorUserId: admin.id,
        action: "demo.profile.shown",
        entityType: "MemberProfile",
        entityId: profile.id,
        metadata: { visibleUntil: window.expiresAt },
        request,
      });
      return NextResponse.json({
        message: "Demo profile visibility enabled.",
    ...
    if (action === "hide") {
      const updated = await prisma.memberProfile.update({
        where: { id: profile.id },
        data: { demoVisible: false, demoVisibleUntil: new Date() },
      });
      await appendAdminAudit({
        actorUserId: admin.id,
        action: "demo.profile.hidden",
        entityType: "MemberProfile",
        entityId: profile.id,
        request,
      });
