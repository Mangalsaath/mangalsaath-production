...
export function normalizeDemoProfileControl(value = {}) {
  return {
    ...
    allowDirectProfileView: asBoolean(
      value.allowDirectProfileView,
      DEFAULT_DEMO_PROFILE_CONTROL.allowDirectProfileView,
    ),
    ...
    viewerAccessRequired: asBoolean(
      value.viewerAccessRequired,
      DEFAULT_DEMO_PROFILE_CONTROL.viewerAccessRequired,
    ),
...
export function isAuthorizedDemoViewer(request, control) {
  ...
  if (!settings.viewerAccessRequired) return true;
  const cookie = request?.cookies?.get?.(DEMO_ACCESS_COOKIE)?.value || "";
  return verifyDemoAccessToken(cookie, settings);
}
