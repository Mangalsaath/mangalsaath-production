...
const SYNTHETIC_DISCLOSURE = "AI-assisted profile";
...
export function presentPublicProfile(profile, user, { detail = false } = {}) {
  ...
  const synthetic = Boolean(profile?.isDemoProfile);
  const publicProfile = {
    ...enriched,
    name: synthetic ? `${enriched.name} · ${SYNTHETIC_DISCLOSURE}` : enriched.name,
    primaryPhotoData: primary?.data || primary?.url || "",
    photoCount: photos.length,
    isSyntheticProfile: synthetic,
    profileDisclosure: synthetic ? SYNTHETIC_DISCLOSURE : "",
  };
  if (detail) {
