...
const PUBLIC_FIELDS = [
  "id", "mangalNumber", "name", "firstName", "lastName", "gender", "age", "placeOfBirth", "timeOfBirth", "maritalStatus", "height",
  "religion", "caste", "subCaste", "gotra", "education", "profession", "annualCtc",
  "fatherName", "fatherStatus", "fatherOccupation", "motherName", "motherStatus", "motherOccupation",
  "familyType", "familyLocation", "familyValues", "brothersDetails", "sistersDetails",
  "brothersMarried", "brothersUnmarried", "sistersMarried", "sistersUnmarried", "country", "state", "city",
  "about", "partnerAgeMin", "partnerAgeMax", "partnerReligion", "partnerCaste", "partnerLocation",
  "partnerMaritalStatus", "partnerEducation", "partnerProfession",
  "initials", "verified", "trustedProfile", "verificationStatus", "updatedAt", "createdAt"
];
...
export function isProfilePublishable(profile, user) {
  ...
  if (profile.isDemoProfile) return isDemoProfileVisible(profile);
...
export function presentPublicProfile(profile, user, { detail = false } = {}) {
  ...
  const publicProfile = {
    ...enriched,
    name: mangalsaathId ? `${enriched.name} · ${mangalsaathId}` : enriched.name,
    profileName: enriched.name,
    mangalsaathId,
